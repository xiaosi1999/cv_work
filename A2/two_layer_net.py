...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        y1 = X.dot(W1) + b1  # N*H
        mask1 = np.zeros_like(y1)
        mask1[y1>0] = 1
        y2 = y1 * mask1
        scores = y2.dot(W2) + b2  # N*C
        ...
        loss = np.sum(loss1)/N + reg * np.sum(W1**2) + reg * np.sum(W2**2)
        ...
        dloss1 = np.ones((N, 1)) / N  # N
        dscores1 = dloss1 * np.exp(scores1) / np.sum(np.exp(scores1), axis=1, keepdims=True)
        dscores1[[range(N)], y] -= dloss1.T
        dscores = dscores1
        dW2 = y2.T.dot(dscores)
        dW2 += 2 * reg * W2
        db2 = np.sum(dscores, axis=0)
        dy1 = dscores.dot(W2.T) * mask1
